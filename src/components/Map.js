...
persons.forEach(person => {
  ...
  .then(data => {
    ...
    else 
    {
      ...
      if (data.length)
      {
        someCountry = data.filter(country => country.name.toLowerCase() === location.toLowerCase())
        if (someCountry.length === 1) // mostly students in india, becus of API returns
        {
          arr.push({lat: someCountry[0].latlng[0], lng: someCountry[0].latlng[1]});
        }
        else if (location === 'united states') // hard coded for some students in usa, becus of API return
        {
          arr.push({lat: data[1].latlng[0], lng: data[1].latlng[1]});
        }
        else if (location === 'uk') //hard coded for some students in uk,  becus of API returns
        {
        ...
        >
          { arr.length !== 0? 
            arr.map((location,i) => 
              <AnyReactComponent
                key={i}
                lat={location.lat + Math.random() + 0.5}
                lng={location.lng+ Math.random() + 0.2}
              >
                <Canvas />
              </AnyReactComponent>
            ) :
