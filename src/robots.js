import React from "react";

export const robots = [
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`data:image/jpeg;base64,___idCIIWoggIIgmCIICRBCiCBP5HypeBA6uI5Oq9ccznOFddyfmvfJPqO/6leCZAbHxzLZv/AMudrSOp1INJh8hy+qF2cXauX3LjwDSWt9aK7mLFqroB80HQhANey6EEfKuV+aqY8dV1XRhZQ9PP0QSystg3rSAqM43/ACV6tjsefuFVnZ5eXRBzZRfluqkzdvXqr8jN/wBbqpO0i9ig5XFb7gkb925ryD6f2tYndeprecated__`}
            />
        ),
        name: "Andrej Zadnik",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/andrejza/"
                target="_blank"
                rel="noopener noreferrer"
            >
                www.hipervision.eu
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/andrej-zadnik-3929233a/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    andrejza
                </a>
            </div>
        ),

        jobTitle: "Full-Stack Dev, C++",

        deprecated_location: "Stuttgart"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`data:image/jpeg;base64,_____iddDWDF0SeHbpYwYnbCq5VP2FWaXMdpNLgnlxFwB1Nc2t5ijTKCdQbKn1q6cF4qLm2kik/7Dr1H0pWi0JFekuJLjXIzFYy3U7AVLBBay48zOcY8oJq2WdjbfV2CgAqvQ4ODUMFtGrHVCpAPUDFFIZIBsrKCdeprecated__idMOoP71kbvFNH5jqZCpkVSQp9/2oAN5UW4kcKxaMkFcSao8dv0qG9ZFeWFo9TQYKqhPMf4HxRtvanVO8jQst1FphUMckgb59f9VHc23NuJJCrI9qpjtXicvLdeprecated______`}
            />
        ),
        name: "Mitch Hankins",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/IronCoderXYZ"
                target="_blank"
                rel="noopener noreferrer"
            >
                www.somedeprecated_website.com
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/IronCoderXYZ/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    username
                </a>
            </div>
        ),

        jobTitle: "Full-Stack JavaScript Dev",

        deprecated_location: "Denver, CO USA"
    },

    {
        deprecated_id: "",
        name: "Piyush Gupta",

        deprecated_website: (
            <div>
                {" "}<a
                href="http://pdshah.com/"
                target="_blank"
                rel="noopener noreferrer"
            >
                Cold Storage
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/piyush-gupta-3a268826"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    linkedin
                </a>
            </div>
        ),

        jobTitle: "Full-Stack Developer",

        deprecated_location: "India"
    },

    {
        deprecated_id: "",
        name: "Gary Hicks",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://www.traceofwind.com"
                target="_blank"
                rel="noopener noreferrer"
            >
                www.traceofwind.com
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/traceofwind/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    traceofwind
                </a>
            </div>
        ),

        jobTitle: "Full-Stack Designer",

        deprecated_location: "London, England"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars2.githubusercontent.com/u/22373538?s=460&v=4`}
            />
        ),
        name: "Dylan Thorwaldson",

        deprecated_website: (
            <div>
                {" "}<a
                href="http://www.dylanthorwaldson.com"
                target="_blank"
                rel="noopener noreferrer"
            >
                Portfolio Site |
            </a>
                <br/>
                <a
                    href="https://www.github.com/dtthor"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    {" "}Github Page
                </a>
                <br/>
                <a
                    href="https://linkedin.com/in/dtthor"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    dtthor
                </a>
            </div>
        ),

        jobTitle: "Fullstack Web Developer",

        deprecated_location: "Austin, TX USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`data:image/jpeg;base64,______id3+yp87aIiCJpvHm783ewXiIpVf/2Q==`}
            />
        ),
        name: "James Granfield",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/googlr"
                target="_blank"
                rel="noopener noreferrer"
            >
                github.com/googlr
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/xiujia-guo-88697181/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    James\'s LinkedIn
                </a>
            </div>
        ),

        jobTitle: "Software Engineer/ Developer",

        deprecated_location: "New York City, NY USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://ktran031.github.io/kt_portfolio/images/khoi.jpg`}
            />
        ),
        name: "Khoi Tran",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://ktran031.github.io/kt_portfolio"
                target="_blank"
                rel="noopener noreferrer"
            >
                Khoi Tran\'s Porfolio
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/khoi-tran-35564991/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Khoi Tran\'s LinkedIn
                </a>
            </div>
        ),

        jobTitle: "Front End Developer",

        deprecated_location: "Orange County, CA USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C5603AQHrnJjaX7E8Sw/profile-displayphoto-shrink_800_800/0?e=1528254000&v=beta&t=PWYVA_XZigZXr4rtNkcdRZiqFyv2IVmoRov0GSepi3A`}
            />
        ),
        name: "Pak Chu",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/spaceforestchu"
                target="_blank"
                rel="noopener noreferrer"
            >
                github.com/spaceforestchu
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/pakchu"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    pakchu
                </a>
            </div>
        ),

        jobTitle: "Fullstack (Javascript Developer)",

        deprecated_location: " New York City, NY USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`http://www.swapnilgaikwad.tech/assets/images/profile_pic1.jpg`}
            />
        ),
        name: "Swapnil Gaikwad",

        deprecated_website: (
            <div>
                {" "}<a
                href="http://www.swapnilgaikwad.tech"
                target="_blank"
                rel="noopener noreferrer"
            >
                github.com/Swapnil2095
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/swapnil-gaikwad-2095"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Swapnil2095
                </a>
            </div>
        ),

        jobTitle:
            "Fullstack Developer \n Data Scientist(BigData, ML, NLP)",

        deprecated_location: "Pune, INDIA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars1.githubusercontent.com/u/32341221?s=460&v=4`}
            />
        ),
        name: "Nabeel Sdeprecated_iddiqui",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/nsdeprecated_iddiqui25"
                target="_blank"
                rel="noopener noreferrer"
            >
                Nabeel\'s Portfolio
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/nsdeprecated_iddiqui25/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Nabeel\'s LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Full-Stack JavaScript Developer",

        deprecated_location: "Chicago, IL USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars2.githubusercontent.com/u/32417757?s=460&v=4`}
            />
        ),
        name: "Martín Comito",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/martincomito"
                target="_blank"
                rel="noopener noreferrer"
            >
                github.com/martincomito
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/mart%C3%ADn-comito-627a32153/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    martincomito
                </a>
            </div>
        ),
        jobTitle: "Front-End Developer",

        deprecated_location: "Buenos Aires, Argentina"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars2.githubusercontent.com/u/17670025?s=460&v=4`}
            />
        ),
        name: "Krishnanath Maly",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/krishnanath"
                target="_blank"
                rel="noopener noreferrer"
            >
                GitHub
            </a>
                <br/>
                <a
                    href="http://www.crzna.com"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    deprecated_website
                </a>
                <br/>
                <a href="https://www.linkedin.com/in/krishnanath/">LinkedIn</a>
            </div>
        ),

        jobTitle: "Full Stack Developer",

        deprecated_location: "Malayattoor KL, INDIA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`http://oudia15.github.io/Images/me_linkedin.jpg`}
            />
        ),
        name: "Oumar Diarra",

        deprecated_website: (
            <div>
                {" "}<a
                href="http://oudia15.github.io"
                target="_blank"
                rel="noopener noreferrer"
            >
                oudia15.github.io
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/diarraoumar/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    diarraoumar
                </a>
            </div>
        ),

        jobTitle: "Full stack Developer",

        deprecated_location: "Louisiana USA (Willing to Relocate)"
    },

    {
        deprecated_id: "",
        name: "James Chhun",

        deprecated_website: (
            <div>
                <a
                    href="https://wingchhun.github.io/portfolio.github.io//"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    James\'s Portfolio deprecated_website
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/james-chhun-16b1b5120/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    James\'s LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Front-End Developer",

        deprecated_location: "San Diego, CA USA"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C5603AQFJuMW6DO74xw/profile-displayphoto-shrink_800_800/0?e=1528477200&v=beta&t=O952AL-tutZW3Z0aAaGIjS7jxLDFST_5Lzodga0_uj0`}
            />
        ),

        name: "Ismael Vazquez",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://iamismael.com"
                target="_blank"
                rel="noopener noreferrer"
            >
                I build great deprecated_websites!
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/ismael-vazquez-jr-0415a1104/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Get to know me
                </a>
            </div>
        ),
        jobTitle: "Full-stack Javascript Developer",

        deprecated_location: "Los Angeles, CA USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C4D03AQEdhmhOXXRi7g/profile-displayphoto-shrink_800_800/0?e=1528477200&v=beta&t=Ibu6t9ZiYsqch12H9_wwvvx4bT8vU95QPAq1Mubl0Dc`}
            />
        ),
        name: "Adrian Wagener",

        deprecated_website: (
            <div>
                <a
                    href="https://wagad22.github.io/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    www.adrian-wagener.com
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/adrian-wagener-45b741134/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    See my cv!
                </a>
            </div>
        ),
        jobTitle: "Fullstack Webdeveloper",

        deprecated_location: "Frankfurt, Germany"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://nadyafa.com/pics/nadezhdafalaleeva.jpg`}
            />
        ),
        name: "Nadezhda Falaleeva",

        deprecated_website: (
            <div>
                <a href="http://nadyafa.com" target="_blank" rel="noopener noreferrer">
                    Hopes91
                </a>
                <br/>
                <a
                    href="https://github.com/hopes91"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Hopes91/Github
                </a>
            </div>
        ),

        jobTitle: "Front-End Developer",

        deprecated_location: "Kirov, Russia"
    },

    {
        deprecated_id: "",
        name: "Chirag Chopra",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/chiggy1997"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    https://github.com/chiggy1997
                </a>
            </div>
        ),

        jobTitle: "Full Stack Web Developer",

        deprecated_location: "Mumbai , India"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://murilomiranda.github.io/img/my_picture.jpg`}
            />
        ),
        name: "Murilo Miranda Nüßlein",

        deprecated_website: (
            <div>
                <a
                    href="https://murilomiranda.github.io/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Murilo\'s Portfolio
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/murilo-miranda-nuesslein"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Murilo\'s LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Front-end Web Development",

        deprecated_location: "Berlin, Germany"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars1.githubusercontent.com/u/2739361?s=460&v=4`}
            />
        ),
        name: "Christine Aqui",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/christine-aqui/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Christine\'s GitHub
                </a>
                <br/>
                <a
                    href="https://ca.linkedin.com/in/christineaqui"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Christine\'s Linkedin
                </a>
            </div>
        ),
        jobTitle: "Full-Stack Javascript Developer",

        deprecated_location: "Toronto, On Canada"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars3.githubusercontent.com/u/8761960?s=460&v=4`}
            />
        ),
        name: "Sheel Parekh",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/RizaneEves"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    RizaneEves
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/sheelparekh"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    sheelparekh
                </a>
            </div>
        ),
        jobTitle: "Full Stack Developer",

        deprecated_location: "Champaign, IL USA (Willing to relocate!)"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`http://rabinrai.com/wp-content/uploads/2017/11/rbn_img-300x225.jpg`}
            />
        ),
        name: "Rabin Rai",

        deprecated_website: (
            <div>
                <a href="http://rabinrai.com" target="_blank" rel="noopener noreferrer">
                    Portfolio Site
                </a>
                <br/>
                <a
                    href="http://github.com/rabinrai44"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    GitHub Profile
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/rabin-rai-58a43913a/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Linkedin Profile
                </a>
            </div>
        ),

        jobTitle: "Front-End Web Developer",

        deprecated_location: "Grand Rapdeprecated_ids, MI USA  (Remote or Onsite)"
    },

    {
        deprecated_id: "",
        name: "Oladele Olorunsola",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/deluxscript"
                target="_blank"
                rel="noopener noreferrer"
            >
                Deluxscript
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/oladele-olorunsola-707a2175"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Oladele Olorunsola
                </a>
            </div>
        ),
        jobTitle: "Full Stack Developer",

        deprecated_location: "Lagos, NGA"
    },

    {
        deprecated_id: "",
        name: "Danny Ferguson",

        deprecated_website: (
            <div>
                <a href="https://dandzn.com" target="_blank" rel="noopener noreferrer">
                    Dandzn.com
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/adannyferguson/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Front-End Developer",

        deprecated_location: "Gaspésie, Québec (Willing to relocate)"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars0.githubusercontent.com/u/35478461?s=460&v=4`}
            />
        ),
        name: "Kirtiben Parekh",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/kirti010"
                target="_blank"
                rel="noopener noreferrer"
            >
                Github Profile
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/kirti010/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Front-End Developer",

        deprecated_location: "San Jose, CA USA (Willing to relocate)"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C4D03AQGcf89ACxun0Q/profile-displayphoto-shrink_800_800/0?e=1528549200&v=beta&t=4-3H7jB8ADFrpOZDrDr9T4ZJhTFl88OVq034R3j4svU`}
            />
        ),
        name: "Piotr Kowalski",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/piotrke"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Github
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/piotr-kowalski-57379a120/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Full-Stack .NET Developer",

        deprecated_location: "Poland"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars2.githubusercontent.com/u/25475754?s=460&v=4`}
            />
        ),
        name: "Kgothatso Desmond",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/DesmondThema"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Github Profile
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/kgothatso-desmond-45b380b2/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Web Developer\n(NodeJs, PHP and Magento)",

        deprecated_location: "Cape Town, South Africa"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars3.githubusercontent.com/u/26254354?s=460&v=4`}
            />
        ),
        name: "Isaac Zhou",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/isaaczhou"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Isaac Zhou
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/isaaczhou/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Chief Data Scientist",

        deprecated_location: "New York City, New York USA"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://abdusdev.me/assets/img/profile.jpg`}
            />
        ),
        name: "Abdus Samad",

        deprecated_website: (
            <div>
                <a href="https://abdusdev.me" target="_blank" rel="noopener noreferrer">
                    Portfolio deprecated_website
                </a>
                <br/>
                <a
                    href="https://github.com/thisisabdus"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Github
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/thisisabdus/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    thisisAbdus
                </a>
            </div>
        ),
        jobTitle: "Web Developer",

        deprecated_location: "Assam, India"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://mohammedmulazada.nl/img/moniac/cutout2.svg`}
            />
        ),
        name: "Mohammed Mulazada",

        deprecated_website: (
            <div>
                <a
                    href="https://mohammedmulazada.nl"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Portfolio deprecated_website
                </a>
                <br/>
                <a
                    href="https://github.com/moniac"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Github
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/mohammed-mulazada-81096b5b/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Front-End Developer",

        deprecated_location: "Amsterdam, Netherlands"
    },

    {
        deprecated_id: "",
        name: "Yuan Li",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/tinauwtron/tinauwtron.github.io"
                target="_blank"
                rel="noopener noreferrer"
            >
                Yuan's Github
            </a>
            </div>
        ),

        jobTitle: "Front End Web Developer",

        deprecated_location: "Toronto, Ontario Canada"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars0.githubusercontent.com/u/8077875?s=460&v=4`}
            />
        ),
        name: "Joe Horn",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/joejhorn"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    GitHub
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/joe-horn-3b306b1/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Front-End Developer",

        deprecated_location: "Reno, Nevada USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://mikerodev.com/assets/images/HeadshotFeb18.jpg`}
            />
        ),
        name: "Michael Robards",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://mikerodev.com"
                target="_blank"
                rel="noopener noreferrer"
            >
                mikerodev.com
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/michael-j-robards-630526138/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Michael J Robards
                </a>
            </div>
        ),
        jobTitle: "Software Engineer",

        deprecated_location: "Atlanta, Ga USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars2.githubusercontent.com/u/740101?s=460&v=4`}
            />
        ),
        name: "Waqar Goraya",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/waqar3"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    github.com/waqar3
                </a>
                <br/>
                <a
                    href="https://linkedin.com/waqargoraya"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    waqargoraya
                </a>
            </div>
        ),
        jobTitle: "Web Developer",

        deprecated_location: "Portsmouth, UK"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`http://malikvishal.me/assets/images/vishal.jpg`}
            />
        ),
        name: "Vishal Malik",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/vishal0027"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    My Github Account
                </a>
                <br/>
                <a
                    href="http://malikvishal.me"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Portfolio deprecated_website
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/vishal0027"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    My Linkedin
                </a>
            </div>
        ),

        jobTitle: "Web Developer",

        deprecated_location: "Toronto, ON Canada"
    },
    {
        deprecated_id: "",
        name: "Yifang Di",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://yifangdi.blogspot.com/"
                target="_blank"
                rel="noopener noreferrer"
            >
                blog
            </a>
                <br/>
                <a
                    href="https://github.com/YvonneD"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Github
                </a>
            </div>
        ),

        jobTitle: "Front-end Web Developer",

        deprecated_location: "Flint, MI USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars0.githubusercontent.com/u/23562296?s=460&v=4`}
            />
        ),
        name: "Zans Litinskis",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://magvin.github.io/"
                target="_blank"
                rel="noopener noreferrer"
            >
                My Portfolio
            </a>
                <br/>
                <a
                    href="https://github.com/Magvin"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Github
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/jeanlitinskis"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),

        jobTitle: "Freelance Front-end Web Developer",

        deprecated_location: "Eastbourne,United Kingdom"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C4D03AQE-1w0mjbpHeA/profile-displayphoto-shrink_800_800/0?e=1528552800&v=beta&t=AQa2nhly_-ikre7ozynBY2YxWEIvo55T1a2Lj2aWB8g`}
            />
        ),
        name: "Els",

        deprecated_website: (
            <div>
                <a
                    href="https://www.bloobloons.com"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    bloobloons
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/bloobloons"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    More about me
                </a>
            </div>
        ),
        jobTitle: "Full Stack Developer",

        deprecated_location: "Portsmouth, UK"
    },

    {
        deprecated_id: "",
        name: "Aminul Islam",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/amin135"
                target="_blank"
                rel="noopener noreferrer"
            >
                https://github.com/amin135
            </a>
            </div>
        ),
        jobTitle: "Full Stack Web Developer",

        deprecated_location: "Totowa, NJ USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C5103AQEnRmgvAyGi8g/profile-displayphoto-shrink_800_800/0?e=1528552800&v=beta&t=OfUQbBBxFjOcvpZYflHT4LS5eBi3ImU1WKJZVG7zRug`}
            />
        ),
        name: "Priyanshu Tyagi",

        deprecated_website: (
            <div>
                {" "}<a
                href="https://github.com/priyanshugithub"
                target="_blank"
                rel="noopener noreferrer"
            >
                Priyanshu\'s Portfolio
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/priyanshutyagi1996/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Priyanshu\'s LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Full-Stack JavaScript Developer",

        deprecated_location: "New Delhi,India (Willing to Relocate)"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars3.githubusercontent.com/u/33874571?s=460&v=4`}
            />
        ),
        name: "Amadi Lucky",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/w3bh4ck"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Lucky\'s Portfolio
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/lucky-amadi-40408b41/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Lucky\'s LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Full-Stack Developer",

        deprecated_location: "Nigeria (Willing to Relocate & Remote)"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://pulamusic.github.io/images/JimHeadshot.jpg`}
            />
        ),
        name: "Jim Carroll",

        deprecated_website: (
            <div>
                <a
                    href="https://pulamusic.github.io/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Jim Carroll\'s Portfolio
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/james-carroll-36001511a/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Jim\'s LinkedIn Page
                </a>
            </div>
        ),
        jobTitle: "Academic &amp; Front-End Developer",

        deprecated_location: "Springfield, MA USA"
    },

    {
        deprecated_id: "",
        name: "Kladeprecated_idas Vysniauskas",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/Kladeprecated_idasVysniauskas"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Kladeprecated_idas Portfolio
                </a>
            </div>
        ),

        jobTitle: "Full-Stack Developer",

        deprecated_location: "Preston, UK"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`data:image/jpeg;base64,/9j/2wCEAAgGBgcGBQgHBwcJCQgKDBQNDAsLDBkSEw8UHRofHh0aHBwgJC4nICIsIxwcKDcpLDAxNDQ0Hyc5PTgyPC4zNDIBCQkJDAsMGA0NGDIhHCEyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMjIyMv/AABEIAMoAygMBIgACEQEDEQH/xAGiAAABBQEBAQEBAQAAAAAAAAAAAQdeprecated________idcCfftUNx/qmPfceaWEnK8/wipaEypGrR3i5xg8E1qqNo3EfL0A9azb0kSLg4yw6VpqBvQY4oj2GhjA7gcAuenoKRUAG0NgdWY96U/df60kv+oiHq1UMCvmjniMdvU1GwAGXOB2Udall/wCPmdeprecated_`}
            />
        ),
        name: "Louise van den Berg",

        deprecated_website: (
            <div>
                <a
                    href="https://louisevdb.herokuapp.com"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Louise\'s Portfolio
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/louise-van-den-berg-016179114/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Louise\'s LinkedIn Page
                </a>
            </div>
        ),
        jobTitle: "Fullstack Web Developer",

        deprecated_location: "Johannesburg - South Africa"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C4D03AQE5FEvf75CqNA/profile-displayphoto-shrink_800_800/0?e=1528552800&v=beta&t=4bxzIqg5f8nRtPzfMlWg_YvxJFqC93lg6EeMRGZHt-0`}
            />
        ),
        name: "Matthew Collier",

        deprecated_website: (
            <div>
                <a
                    href="https://github.com/matthewcollier1/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    GitHub
                </a>
                <br/>
                <a
                    href="http://matthewcollier.ca"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Portfolio
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/matthewjcollier/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Front-end Web Developer",

        deprecated_location: "Toronto, Canada"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C5103AQF1djmtZya33g/profile-displayphoto-shrink_800_800/0?e=1528552800&v=beta&t=9--dS-8c87a4kSgCkKV6hEf3Q9eTSPJIlHc2sQrwsUM`}
            />
        ),
        name: "Sri Harsha",

        deprecated_website: (
            <div>
                <a
                    href="http://sriharsha.ml/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    www.sriharsha.ml
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/harsha444"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Sri Harsha
                </a>
            </div>
        ),
        jobTitle: "Full Stack Developer",

        deprecated_location: "Durgapur,India"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C4D03AQEpXEA2S8bKjg/profile-displayphoto-shrink_800_800/0?e=1528552800&v=beta&t=-eWTij44N-3VxkLASMQPuJQaD7-udPu-vZ94zHmcldU`}
            />
        ),
        name: "Yuankan Fang",

        deprecated_website: (
            <div>
                <a
                    href="https://kyleyk.github.io/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Portfolio
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/yuankanf/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    LinkedIn
                </a>
            </div>
        ),
        jobTitle: "Web Developer/Software Engineer",

        deprecated_location: "San Diego, CA USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`http://jonathanitzen.com/wp-content/themes/jonathanportfolio/imgs/profile.jpg`}
            />
        ),
        name: "Jonathan Itzen",

        deprecated_website: (
            <div>
                {" "}<a
                href="http://jonathanitzen.com/"
                target="_blank"
                rel="noopener noreferrer"
            >
                www.jonathanitzen.com
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/jonathan-itzen"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    username
                </a>
            </div>
        ),

        jobTitle: "WordPress Developer",

        deprecated_location: "Fort Dodge, Iowa USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C4E03AQFG-tPI_99VdQ/profile-displayphoto-shrink_800_800/0?e=1528552800&v=beta&t=Pwh88C9Q1ZyLh-kTyVtsUSD48WM22TnxWyLu4ALDg0I`}
            />
        ),
        name: "Ryan Walker",

        deprecated_website: (
            <div>
                <a
                    href="https://ryanwalkerdevelopment.com"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Portfolio
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/ryan-walker-41082b47/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Linkedin Profile
                </a>
            </div>
        ),
        jobTitle: "Front-end Developer",

        deprecated_location: "Youngstown, OH USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars0.githubusercontent.com/u/25121780?s=460&v=4`}
            />
        ),
        name: "Daniel Alviso",

        deprecated_website: (
            <div>
                <a
                    href="http://dan-alviso.com"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Portfolio Site
                </a>
                <br/>
                <a
                    href="http://github.com/dan-alviso"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    GitHub
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/dan-alviso/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Linkedin
                </a>
            </div>
        ),
        jobTitle: "Software Developer",

        deprecated_location: "San Francisco Bay Area, CA USA"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://media.licdn.com/dms/image/C4E03AQFJo9NvT8gtAg/profile-displayphoto-shrink_800_800/0?e=1528549200&v=beta&t=DRZ5Q1-o5SWRSH3j-W8dHwfM6UlWCr813IsLSGuAVaQ`}
            />
        ),

        name: "Osei Kofi Nimoh",

        deprecated_website: (
            <div>
                {" "}<a
                href="http://vitusbtravelntour"
                target="_blank"
                rel="noopener noreferrer"
            >
                Portfolio Site
            </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/kofi-osei-nimoh-15148a73/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Linkedin
                </a>
            </div>
        ),

        jobTitle: "Web Developer",

        deprecated_location: "Accra, Ghana"
    },

    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`YOUR IMG LINK HERE `}/>
        ),

        name: "Petar Dyakov",

        deprecated_website: (
            <div>
                <a href="dyakov.cc" target="_blank" rel="noopener noreferrer">
                    Portfolio Site
                </a>
                <br/>
                <a
                    href="www.linkedin.com/in/petar-dyakov"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Linkedin
                </a>
            </div>
        ),

        jobTitle: "Web Developer",

        deprecated_location: "Sofia, Bulgaria"
    },
    {
        deprecated_id: <img className="br-100 pa1 ba b--black h4 w4" alt='robots'
                            src={`https://avatars2.githubusercontent.com/u/29685827?s=460&v=4`}/>,

        name: "Wolfgang Kreminger",

        deprecated_website: <div><a href="https://github.com/r4pt0s" target="_blank" rel="noopener noreferrer">My GitHub
            repository</a>
            <br></br>
            <a href="linkedin.com/in/wolfgang-kreminger-43386b151" target="_blank" rel="noopener noreferrer">Look at my
                references on LinkedIn</a>
        </div>,

        jobTitle: "Full Stack Developer",

        deprecated_location: "Austria, Burgenland"
    },
    {
        deprecated_id: <img className="br-100 pa1 ba b--black h4 w4" alt='robots'
                            src={`https://www.google.co.in/imgres?imgurl=https%3A%2F%2Fmedia-exp2.licdn.com%2Fmpr%2Fmpr%2Fshrinknp_200_200%2FAAIA_wDGAAAAAQAAAAAAAA2eAAAAJDBlNjVlYWEzLTlhNDUtNDZkZS1iN2ZhLWIwMjEwYThiMmUzMg.jpg&imgrefurl=https%3A%2F%2Fin.linkedin.com%2Fin%2Fdarryl-ferdinands-43b89671&docdeprecated_id=qpyLR1KLBcN3kM&tbndeprecated_id=aPGNGNvpji-xLM%3A&vet=10ahUKEwiV4anwyMPaAhWBNI8KHWVTCdsQMwg1KAAwAA..i&w=200&h=200&itg=1&bih=626&biw=1366&q=Darryl%20Ferdinands&ved=0ahUKEwiV4anwyMPaAhWBNI8KHWVTCdsQMwg1KAAwAA&iact=mrc&uact=8`}/>,

        name: "Darryl Steve Ferdinands",

        deprecated_website: <div><a href="https://github.com/darrylferdinands" target="_blank"
                                    rel="noopener noreferrer">My GitHub
            Account</a>
            <br></br>
            <a href="https://www.linkedin.com/in/darryl-ferdinands-43b89671/" target="_blank" rel="noopener noreferrer">LinkedIn
                Profile</a>
            <a href="https://darrylferdinands.wixsite.com/education" target="_blank" rel="noopener noreferrer">My
                Portfolio</a>
        </div>,

        jobTitle: "Full Stack Developer",
        deprecated_location: "New Delhi, India"
    },
    {
        deprecated_id: "",
        name: "Amaechi Chuks",

        deprecated_website:
            <div><a href="https://github.com/amaechi-chuks" target="_blank" rel="noopener noreferrer">My GitHub
                Account</a>
                <br></br>
                <a href="https://www.codewars.com/users/amaechi-chuks" target="_blank" rel="noopener noreferrer">CodeWar
                    Profile</a>
                <a href="https://amaechi-chuks.github.io/part/" target="_blank" rel="noopener noreferrer">My
                    Portfolio</a>
            </div>
        ,

        jobTitle: "Full Stack Developer",
        deprecated_location: "Lagos, Nigeria"
    },
    {
        deprecated_id: (
            <img
                className="br-100 pa1 ba b--black h4 w4"
                alt="robots"
                src={`https://avatars3.githubusercontent.com/u/37252418?v=4`}
            />
    ),
    name: "Jacinto Wong",
        deprecated_website: (
            <div>
                <a
                    href="https://github.com/JacintoDesign"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    GitHub
                </a>
                <br/>
                <a
                    href="https://jacinto.design"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    deprecated_website
                </a>
                <br/>
                <a
                    href="https://www.linkedin.com/in/jacintowong/"
                    target="_blank"
                    rel="noopener noreferrer"
                >
                    Linkedin
                </a>
            </div>
    ),
    jobTitle: "Full-Stack Developer",
        deprecated_location: "Toronto, Canada"
    },
    {
        deprecated_id: <img className="br-100 pa1 ba b--black h4 w4" alt='robots'
                            src={`https://www.fsdev.ml/myself.jpg`}/>,

        name: "Thomas Dreyer",

        deprecated_website: <div><a href="https://www.fsdev.ml" target="_blank" rel="noopener noreferrer">Portfolio
            Site</a><br/><br/>
            <a href="https://github.com/thomasdreyer" target="_blank" rel="noopener noreferrer">Github</a>
    <br></br>
            <a href="https://www.linkedin.com/in/thomas-dreyer" target="_blank" rel="noopener noreferrer">Linkedin</a>
        </div>,
        jobTitle: "Full Stack Developer",
        deprecated_location: "Johannesburg, South Africa"
    },
    {
        id: 57,
        name: "Thimothé Lamoureux",
        img: "",
        links: {
            website: "http://www.wcubemedia.com",
            linkedin: "https://www.linkedin.com/in/thimothe-lamoureux",
            github: "https://github.com/aparadox"
        },
        jobTitle: "Web Developer",
        location: {
            city: "Whitehorse",
            state: "Yukon",
            country: "Canada"
        }
    }
];
